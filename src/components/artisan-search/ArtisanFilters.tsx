...
const ArtisanFilters = ({
  ...
  return <div className="bg-white rounded-2xl p-6 shadow-soft border border-border sticky top-24">
      ...
      <div className="mb-6">
        ...
        <div className="relative">
          ...
          <Input placeholder="Rechercher une ville..." value={citySearch} onChange={e => {
          setCitySearch(e.target.value);
          setShowCitySuggestions(true);
          if (e.target.value === "") {
            setSelectedCity("");
          }
        }} onFocus={() => setShowCitySuggestions(true)} onBlur={() => setTimeout(() => setShowCitySuggestions(false), 200)} className="pl-10" />
          {showCitySuggestions && citySearch && filteredLocations.length > 0 && <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-border rounded-lg shadow-lg z-50 max-h-48 overflow-y-auto">
              {filteredLocations.map(loc => <button key={loc.value} type="button" onClick={() => handleLocationSelect(loc)} className="w-full px-4 py-2 text-left hover:bg-muted transition-colors text-sm flex items-center gap-2">
                  <span className={`text-xs px-1.5 py-0.5 rounded ${loc.type === 'region' ? 'bg-primary/10 text-primary' : loc.type === 'department' ? 'bg-accent/10 text-accent' : 'bg-muted text-muted-foreground'}`}>
      ...
      <div className="mb-6">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !interventionDate && "text-muted-foreground")}>
              <CalendarIcon className="mr-2 h-4 w-4" />
              {interventionDate ? format(interventionDate, "PPP", {
              locale: fr
            }) : "Sélectionner une date"}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0 bg-white z-50" align="start">
            <Calendar mode="single" selected={interventionDate} onSelect={setInterventionDate} initialFocus locale={fr} disabled={date => date < new Date()} className="pointer-events-auto" />
          </PopoverContent>
        </Popover>
      </div>
