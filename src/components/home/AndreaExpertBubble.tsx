...
const AndreaExpertBubble = () => {
  ...
  const handleAsk = async () => {
    ...
    if (cacheRef.current[trimmed]) {
      setMessages((prev) => [...prev, { role: "andrea", text: cacheRef.current[trimmed] }]);
      return;
    }
    ...
    try {
      ...
      const response = data?.answer || "Je n'ai pas pu répondre. Reformulez votre question.";
      cacheRef.current[trimmed] = response;
      setMessages((prev) => [...prev, { role: "andrea", text: response }]);
    } catch {
