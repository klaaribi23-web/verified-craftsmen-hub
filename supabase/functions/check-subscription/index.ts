  ...
  try {
    ...
    if (customers.data.length === 0) {
      ...
      await supabaseClient
        .from("artisans")
        .update({ subscription_tier: "free", stripe_customer_id: null, subscription_end: null, display_priority: 100 })
        .eq("user_id", user.id);

      return new Response(
        JSON.stringify({ subscribed: false, subscription_tier: "free", subscription_end: null, subscription_start: null, billing_interval: null, payment_method: null }),
    ...
    if (subscriptions.data.length === 0) {
      ...
      await supabaseClient
        .from("artisans")
        .update({ subscription_tier: "free", stripe_customer_id: customerId, subscription_end: null, display_priority: 100 })
        .eq("user_id", user.id);
    ...
    try {
      ...
      if (paymentMethods.data.length > 0) {
        ...
        if (card) {
          paymentMethod = { last4: card.last4, brand: card.brand, exp_month: card.exp_month, exp_year: card.exp_year };
        }
      }
    } catch (pmError) {
